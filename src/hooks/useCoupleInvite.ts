...
export function inviteReasonMessage(reason: string): string {
  const m: Record<string, string> = {
    ...
    // 0024: 양쪽 다 혼자 쓰면서 각자 데이터를 쌓은 경우. 합치기는 병합 문제라 자동으로 하지 않는다 —
    // 조용히 한쪽을 잃는 것보다 이렇게 말하는 편이 낫다.
    HAS_OWN_DATA:
      '이미 혼자 담아둔 장소·일정이 있어요. 지금 연결하면 한쪽 기록만 남아요. [우리]에서 내보내기로 백업한 뒤, 반대로 상대에게 코드를 받아 연결해 주세요.',
  }
