...
describe('migration 0017 — 좌표 암호화 RPC + 파기 잡', () => {
  ...
  it('record_points는 RECORDING 상태가 아니면 거부한다(동의/상태 게이트)', () => {
    expect(sql()).toMatch(/IF\s+v_status\s*<>\s*'RECORDING'\s+THEN[\s\S]*RAISE EXCEPTION/i)
  })

  it('get_session_points: 복호 read + 상대 열람 시 PROVIDE 확인자료', () => {
    const s = sql()
    ...
    expect(s).toMatch(/v_caller\s*<>\s*v_owner[\s\S]*'PROVIDE'/i)
  ...
  it('purge_location_data는 route_points·trip_sessions를 하드 DELETE한다', () => {
    const s = sql()
