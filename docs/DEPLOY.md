...
```bash
node scripts/seed-demo.mjs   # 장소 4곳 · 여행 1개 · 방문 4건 · 일정 3건 · 실측 동선 60점
```

빈 앱은 "기능을 확인할 수 없다"로 반려되는 흔한 사유다. 특히 이 앱의 핵심(동선·전국 지도)은
데이터가 없으면 빈 상태 문구만 나온다.

**service_role이 아니라 데모 계정의 로그인 토큰으로 넣는다.** 그래야 RLS를 그대로 통과하며
심는 것이라 심사관이 보는 것과 정확히 같은 데이터가 된다(관리자 권한으로 우겨 넣으면
RLS 밖의 행이 생겨 실제로는 안 보일 수 있다). 멱등이라 여러 번 돌려도 쌓이지 않는다.
