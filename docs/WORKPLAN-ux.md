...
| | 이유 |
|---|---|
| ~~J 오프라인 큐 연동~~ | **완료**(`ed1abbc`) — 후속 지시로 편입 |
| ~~K 인앱 활동 피드~~ | **완료**(`ed1abbc`). 비평가 지적대로 범위를 "추가/기록"까지로 좁혀 구현하고, 삭제·변경은 구조적으로 불가함을 코드·문구에 명시 |
| ~~22시간 일정 버그~~ | **완료**(`ed1abbc`) — `followEndTime`으로 시작 이동 시 종료가 따라온다 |
| L EventSheet 점진 공개 | 여전히 제외(폼 9개 컨트롤 한번에 노출 · 저장 버튼 sticky 아님). 여기 딸려 있던 22시간 버그만 떼어 고쳤다 |
