...
> ⚠️ **부분 대체됨(2026-08-08)** — 표면 팔레트 결정은 유효하지 않다.
> 배경까지 핑크(`#fff1f4`)로 칠한 탓에 화면 전체가 한 색조가 되어 표면 위계가 서지 않았고,
> 제품이라기보다 습작으로 읽혔다. 지금은 **중성 배경 + 핑크는 액션·강조 전용**이며
> 토큰은 OKLCH로 authoring한다(`src/styles/tokens.css` 머리주석 참조).
> 이 문서의 나머지(색 단독 금지, 반경 6단계, 폰트 self-host, 대비 게이트 존재 등)는 그대로 유효하다.
