...
| # | 증상 | 상태 |
|---|---|---|
| **C1** | **핀을 누르면 지도가 그 핀을 시트 뒤로 숨긴다** — `panTo`는 전체 뷰포트 중앙 기준인데 선택 시 시트가 half(50%)로 승격됨. 매번 발생, full이면 완전히 사라짐 | ✅ `contentPadding`(map_view) + 선택 시 카메라 이동 배선. 2026-08-31 시뮬레이터에서 실측 확인(757pt → 195pt) |
